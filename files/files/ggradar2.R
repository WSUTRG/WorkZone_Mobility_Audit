ggradar2 = function (plot.data, font.radar = "Circular Air Light", values.radar = c("0%", 
                     ...
                     group.point.size = 6, group.colours = NULL, background.circle.colour = "#D7D6D1", 
                     ...
                     legend.text.size = grid.label.size) 
{
    ...
    plot.data[, 1] <- factor(as.character(plot.data[, 1]), levels = unique(plot.data[,1]))
    # print(str(plot.data))
    ...
    if (min(plot.data[, -1],na.rm = TRUE) < centre.y) 
        return("Error: plot.data' contains value(s) < centre.y")
    if (max(plot.data[, -1],na.rm = TRUE) > grid.max) 
    ...
    base <- base + geom_path(data = axis$path, aes(x = x, y = y, 
                                                   group = axis.no), colour = axis.line.colour, size=grid.line.width) ### added size here
